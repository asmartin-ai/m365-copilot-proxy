...
4. Ladder state machine in `packages/core/src/session.ts` (owns sendChat/optionsSets, ~line 531): on session open walk channels — 0: custom-instructions textarea (PRIMARY; server applies to raw agent-less WS turns by default, `add_custom_instructions` flag NOT required), 1: optionsSets gate flag (request-shape control only, not load-bearing). Pass latches the channel for the session; fail retries under a rotated throttle profile (rate/jitter/payload shape); trip a per-channel circuit breaker after N.
5. Honest degrade: every breaker open → stamp `system_fingerprint: 'unsteered'`; never silently ship uninjected output. Values: `unsteered` | `steered:channel=textarea` (primary; alias `steered:channel=custom-instr` for the flag-only path). First-class OpenAI-compatible field: top-level `system_fingerprint` on chat-completion JSON and the Responses envelope, sourced from CopilotSession steering state; mirror as `x_m365_system_fingerprint` in usage for streamed chunks (usage-builder pattern).
