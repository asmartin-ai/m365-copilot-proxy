...
- **Second Copilot surface (fork-only)**: `cowork.ts` + `cowork-protocol.ts` — a Socket.IO probe against `edge.skype.com/registrar` with `containerConfig="…model=fable-5:claude"`; a separate, fork-originated reverse-engineering thread (likely the LAN remote's shared work).
