...
# sessions, or auth.
#
#   bun run pi                    # interactive, default model m365-copilot
#   PORT=24034 bun run pi         # point at a proxy on a different port
#   MODEL=quick bun run pi        # different default model
#   bun run pi -- -p "write hi"   # pass a prompt / extra pi flags through
#
# Start the proxy first in another shell:  bun run proxy   (defaults to :4141)
set -euo pipefail
...
{"providers":{"m365":{"api":"openai-completions","apiKey":"m365","baseUrl":"$BASE","compat":{"supportsDeveloperRole":false,"supportsReasoningEffort":false,"supportsUsageInStreaming":false},"models":[
  ...
  {"id":"gpt-5.6-think-deeper","name":"GPT-5.6 Think Deeper"},
...
if ! curl -s --max-time 3 "http://localhost:${PORT}/health" >/dev/null 2>&1; then
  echo "[pi-local] ⚠ no proxy answering on :${PORT} — start it with 'bun run proxy' (or set PORT=...)"
fi
